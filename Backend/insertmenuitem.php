...
if(in_array($_SESSION['staffloginpermit'], $allowedperms))
	...
	<?php 
		while ( $row = $result-> fetch() )
			{
				echo '<tr><td> ' . $row['dbmenuid'] . '</td><td> ' . $row['dbcatid'] . 
				'</td><td> ' . $row['dbmenuname'] . 
				'</td><td> ' . $row['dbmenuprice'] . 
				'</td><td> ' . $row['dbmenudescr'] . 
				'</td><td> ' . $row['dbmenuinventory'] . 
				'</td><td> ' . $row['dbmenuactive'] . '</td></tr>';
			}
		?>
	</table>
<?php
